<template>
  ...
  <el-table
    ...
    <el-table-column label="课程状态" width="340">
...
export default {
    ...
    methods:{  //创建具体的方法，调用teacher.js定义的方法
        //讲师列表的方法
        getList(page = 1) {
          if(this.CourseQuery.status === 1){
              this.CourseQuery.status = 'Normal'
          } else  if(this.CourseQuery.status === 2){
            ...
            course.getListCourse(this.page,this.limit,this.CourseQuery)
                ...
                // .catch(error =>{ 可以不用写catch 因为response封装了
                //   console.log(error)
                // })
        },
        resetData() { //清空
          //表单输入项数据清空
          this.CourseQuery = {}  //原因这里是双向绑定 对象为空 表单上数据也为空
          this.getList()
        },
        
        removeDataById(id) { //逻辑删除讲师
            this.$confirm('此操作将永久删除课程, 是否继续?', '提示', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              course.deleteCoureId(id)
                .then(response =>{
                  //提示信息
                  this.$message({
                      type: 'success',
